...
- **`<元数据>`:** 包含规约的元信息。
  - **`<标题>`:** 人类可读的标题。
  - **`<摘要>`:** 规约的目标摘要。
  - **`<版本>` (*可选*):** 用于追踪规约自身的修订历史。初始版本为 `1.0`。每次通过“规约修订流程”进行重大修改后，版本号都应增加（例如 `1.1`, `2.0`）。

### 3.2 实施计划标签
- **`<实施计划>`:** 包含规约将如何被实现的技术蓝图。
- **`<阶段化开发>` (*强制*):**
  - **描述:** 这是实施计划的核心，它强制将所有功能划分为核心和次要两个阶段。此标签旨在确保我们遵循“价值驱动的增量交付”原则。
  - **`<核心功能>` (*强制*):**
    - **描述:** 包含此规约中最关键的、必须最先被实现的功能。此标签下必须至少包含一个 `<功能>`。
  - **`<次要功能>` (*可选*):**
    - **描述:** 包含那些重要的、但在核心功能完成后才能开始开发的功能。此标签可以为空。
...
<规约 UID="spec-001">
  ...
  <实施计划>
    ...
    <!-- 旧的、扁平的功能列表将被弃用 -->
    <功能 UID="spec-001-func-001-deprecated">
      <名称>用户登录</名称>
      <描述>处理用户的用户名和密码验证。</描述>
