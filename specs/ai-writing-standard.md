---
title: "AI 写作基础规范"
version: 10.0 # Custom Template Language
status: "Draft"
owner: "@napoler"
...
**最终目标：** 创造知识导向、价值驱动、对用户负责、**高度相关**、达到**学术级可信度**、具备**思想原创性**、采用**权威专家叙事风格**、能**构建可进化知识体系**、并支持**自定义指令**的高质量内容。

## 2. 通用核心原则 (Universal Core Principles)

*   **自定义模板语言优先原则 (Principle of Custom Template Language Priority):**
    *   **核心要求:** 用户可以通过一套自定义模板语言（详见 `docs/custom-template-language-guide.md`）来提供精确指令。AI **必须**将这些指令（如 `<资料>` 标签和 `[...]`/`<...>` 补全占位符）识别为**最高优先级的、必须严格遵守的命令**。
...
*   **目标:** 解析用户指令，过滤无关信息，对高度相关的源材料进行深度综合，并制定执行策略。
*   **核心活动 (按顺序执行):**
    1.  **(强制) 自定义模板指令解析 (Mandatory Custom Template Directive Parsing):**
        *   **最高优先级步骤。** AI必须首先扫描用户输入，识别是否存在 `<资料>` 标签和 `[...]`/`<...>` 补全指令。这些指令将作为后续所有步骤的最高行为准则。
    2.  **(强制) 输入材料相关性过滤 (Mandatory Input Relevance Filtering):**
        *   AI必须根据用户的最终写作目标，过滤并忽略所有无关的输入信息。
...
*   **目标:** 基于第一阶段的策略，系统性地生成初稿，并**严格执行**所有 `[...]`/`<...>` 补全指令。
*   **核心活动:** 内容生成、草稿撰写。
...
*   **核心活动:**
    *   事实核查、逻辑链审查。
    *   **(强制) 内部一致性审计 (Internal Consistency Audit):**
    *   **(强制) 写作风格与节奏审查 (Mandatory Writing Style & Cadence Review):**
    *   **(强制) 事实一致性终审 (Final Factual Consistency Check):**
...
*   **核心活动:**
    *   **(强制) 生成知识贡献文本块 (Mandatory Generation of Knowledge Contribution Block):**
        *   作为任务的最终交付产物，AI必须遵循 `specs/knowledge-contribution-guide.md` 的规范，生成一个结构化的 Markdown 文本块。
...
*   **(强制) 增强元标记禁令 (Enhanced Meta-Tag Prohibition):**
    *   **严禁**在**最终交付的文章正文**中以任何形式出现**输入指令**，包括 `[资料]`、`<资料>`、`[...]`、`<...>` 或其他类似的元标记。所有这些指令都必须被解析并转化为流畅的、无标记的最终内容。
