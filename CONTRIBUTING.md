...
## 9. 实时开发模式 (Live Development Mode)

为了最大化开发效率并实现“快速失败”，我们提供一个“守护”进程，它能在后台持续监控你的文件变更，并自动触发相应的质量检查、测试和构建。

这是一种**强烈推荐**的开发模式，因为它将问题反馈的周期从“提交时”缩短到了“保存时”。
...
启动后，守护进程将执行以下操作：
*   **当 `.py` 文件被修改时**: 自动运行 `ruff` (linting), `black` (formatting), 和 `pytest` (tests)。
*   **当 `.md` 文件被修改时**: 自动运行 `mkdocs build` 来更新文档网站。
*   **当 `pyproject.toml` 被修改时**: 自动为新增的依赖运行 `generate_api_docs.py`，实现API文档的自动生成。
